      ...
      if idx:
        ...
        array = re.search(
          'var {nfunc}\s*=\s*(\[.+?\])'.format(
            nfunc=re.escape(function_match.group(1))), js)
        if array:
          array = array.group(1).strip("[]").split(",")
          array = [x.strip() for x in array]
          return array[int(idx)]

  raise RegexMatchError(caller="get_throttling_function_name",
                        pattern="multiple")
...
def get_throttling_function_code(js: str) -> str:
  ...
  code_lines_list = find_object_from_startpoint(js,
                                                match.span()[1]).split('\n')
